/* ============================================================
   JAFFER HAMEED — hero orchestration
  ...
  if (wk && typeof heroContent === "object" && heroContent.works) {
    ...
    projects.forEach((p, i) => {
      ...
      glass.innerHTML =
        ...
        (p.url ? `<a class="wk-live" href="${p.url}" target="_blank" rel="noopener">Live Site ${ARROW}</a>` : `<span class="wk-status">${p.status || "In Development"}</span>`) +
        `</div>` +
        "</div>" +
    ...
    const openDetail = (data) => {
      ...
      const existingLink = detail ? detail.querySelector(".wk-detail-link") : null;
      if (existingLink) existingLink.remove();
      if (detail && (data.url || data.status)) {
        const link = document.createElement(data.url ? "a" : "span");
        link.className = "wk-detail-link";
        link.textContent = data.url ? "Visit live project →" : (data.status || "In development");
        if (data.url) { link.href = data.url; link.target = "_blank"; link.rel = "noopener"; }
        detail.append(link);
      }
